...
| `Plugins/` | First-party reference Extensions: Sim, Swarm, Bridge, and Detox; Browser/Chromium source for the separately signed app edition; plus the parked AppDock experiment. | Extensions are separate executables using the same public API available to third parties. Optional app-edition host components are explicit exceptions and must remain narrowly reviewed. |
