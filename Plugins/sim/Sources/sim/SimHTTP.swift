...
final class SimHTTPServer {
    ...
    typealias Handler = (Request, _ respond: @escaping (Int, Any) -> Void) -> Void
    ...
    private let queue = DispatchQueue(label: "sim.http")
    private let connectionQueue = DispatchQueue(
        label: "sim.http.connections",
        qos: .userInitiated,
    ...
    @discardableResult
    func start(preferredPort: UInt16) -> Bool {
        let firstPort = Int(preferredPort)
        for rawPort in firstPort..<min(65_536, firstPort + 16) {
            ...
            guard bound == 0, listen(fd, 16) == 0 else { close(fd); continue }
            listenFD = fd
            port = Int(candidate)
            break
        }
    ...
    private func handle(fd: Int32) {
        ...
        if path != "/health" {
            let presented = lines.compactMap { line -> String? in
                ...
                if lower.hasPrefix("x-cmdy-sim-token:") {
                    return text.dropFirst("x-cmdy-sim-token:".count)
                        .trimmingCharacters(in: .whitespaces)
                }
                return nil
            }.first
