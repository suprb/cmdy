...
final class DockHTTPServer {
    ...
    private let queue = DispatchQueue(label: "appdock.http")
    private let connectionQueue = DispatchQueue(
        label: "appdock.http.connections",
        qos: .userInitiated,
    ...
    private func handle(fd: Int32) {
        ...
        if path != "/health" {
            let presented = lines.compactMap { line -> String? in
                ...
                if lower.hasPrefix("x-cmdy-appdock-token:") {
                    return text.dropFirst("x-cmdy-appdock-token:".count)
                        .trimmingCharacters(in: .whitespaces)
                }
                return nil
            }.first
