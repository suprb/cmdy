...
const Code = Object.assign(function Code({ children }: { children: string }) {
          ...
          {groups.map((group) => (
            <div className="docs-nav-group" key={group}>
              ...
              {visible.filter((section) => section.group === group).map((section) => (
                <a href={`#${section.id}`} key={section.id}><EditorialText>{section.title}</EditorialText></a>
              ))}
            </div>
          ))}
        </nav>
      ...
      <div className="docs-content" id="top">
        <div className="docs-mobile-tools">
          ...
          <label className="docs-mobile-nav">
            ...
            >
              ...
              {groups.map((group) => (
                <optgroup label={editorialCaseString(group)} key={group}>
                  {visible.filter((section) => section.group === group).map((section) => (
                    <option value={section.id} key={section.id}>{editorialCaseString(section.title)}</option>
                  ))}
                </optgroup>
              ))}
            </select>
        ...
        {visible.length ? visible.map((section, index) => (
          <section className="doc-section" id={section.id} key={section.id}>
            ...
            <SacredWindow>
              <SacredCard mode="left" title={<EditorialText>{section.title}</EditorialText>}>
                <div className="doc-copy"><EditorialText>{section.content}</EditorialText></div>
              </SacredCard>
            </SacredWindow>
          </section>
        )) : (
          <SacredWindow className="docs-empty">
            <p>no manual section matches “<span className="case">{query}</span>”.</p>
